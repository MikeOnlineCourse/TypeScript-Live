...
const queryParse = (query: string): string => {
...
const parseFindNewQuery = (params: string, fist: number): string => {
...
// 添加 query 參數 push
const routerQueryPush = (params: string): void => {
...
// 添加 query 參數 replace
const routerQueryReplace = (params: string): void => {
  const { pathname } = window.location;
  const query = queryParse(params);
...
// 刪除 query 參數 push
const routerQueryRemove = (params: string): void => {
  const { search } = window.location;
...
// 刪除 query 參數 replace
const routerQueryReplaceRemove = (params: string): void => {
  const { search } = window.location;
  const fist = search.indexOf(params);
  if (["?", "&"].includes(search[fist - 1])) {
    const newQuery = parseFindNewQuery(params, fist);
